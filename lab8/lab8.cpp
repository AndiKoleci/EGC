...
void Lab8::Init()
{
    ...
    // Light & material properties
    {
        ...
        lightPosition[1] = glm::vec3(1, 1.5, 1);
        lightPosition[2] = glm::vec3(2, 1, 1);
...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(1, 0, 0)); //culoare
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0, 1, 0)); //culoare
    ...
    // Render ground
    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.5, 0.5, 0.5)); //culoare
    ...
    // Render the point light in the scene
    {
        ...
        modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, lightPosition[2]);
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.1f));
        RenderMesh(meshes["sphere"], shaders["Simple"], modelMatrix);
    }
}
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    glUniform3fv(light_position, 3, glm::value_ptr(lightPosition[0]));
    ...
    int spotlight_loc = glGetUniformLocation(shader->program, "isInSpotlight");
    glUniform1i(spotlight_loc, isInSpotlight);
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        for (int i = 0; i < 3; i++) {
            if (window->KeyHold(GLFW_KEY_W)) lightPosition[i] -= forward * deltaTime * speed;
            if (window->KeyHold(GLFW_KEY_A)) lightPosition[i] -= right * deltaTime * speed;
            if (window->KeyHold(GLFW_KEY_S)) lightPosition[i] += forward * deltaTime * speed;
            if (window->KeyHold(GLFW_KEY_D)) lightPosition[i] += right * deltaTime * speed;
            if (window->KeyHold(GLFW_KEY_E)) lightPosition[i] += up * deltaTime * speed;
            if (window->KeyHold(GLFW_KEY_Q)) lightPosition[i] -= up * deltaTime * speed;
        }
